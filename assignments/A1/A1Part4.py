import sys
import os
sys.path.append('/mnt/disk1/sms-tools-master/')
...
def downsampleAudio(inputFile, M):
    ...
    """
    sr,y = wavread(inputFile)
    time = len(y)/sr
    down_sampled = hopSamples(y,M)
    input_file = inputFile.split("/")[-1].split(".")[0]
    print(input_file)
    wavwrite(down_sampled, int(sr/M),input_file + "_downsampled.wav")


if __name__=='__main__':

    downsampleAudio("/mnt/disk1/sms-tools-master/sounds/vibraphone-C6.wav", 16)
